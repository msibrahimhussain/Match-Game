...
class App extends Component {
  ...
  onClk = id => {
    ...
    if (cId === id) {
      let n = (Math.random() * 100) / 3
      if (n > 29) {
        n -= 3
      }
      this.setState({
        imageURL: imagesList[Math.ceil(n)].imageUrl,
        cId: imagesList[Math.ceil(n)].id,
      })
  ...
  getFilteredProjects = () => {
    ...
    const filteredProjects = imagesList.filter(
      eachItemDetails => eachItemDetails.category === activeTabId,
    )
  ...
  render() {
    const {imageURL, activeTabId, gameOver, timer, score} = this.state
          ...
          <ul className="sub-header">
            ...
            <li className="timerBoard">
              <img
                ...
                className="timer"
              />
              <p className="scoreBoard">{timer} sec</p>
            </li>
          </ul>
        </nav>
        {!gameOver && (
          <div className="container">
            <img src={imageURL} alt="match" className="thumbnail" />
